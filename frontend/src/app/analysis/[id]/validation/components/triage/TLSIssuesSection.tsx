...
import type { TLSIssue } from "@/lib/types";
...
const issueTypeLabels: Record<string, string> = {
  ...
  missing_tls_secret: "Missing TLS Secret",
};

export function TLSIssuesSection({ issues }: { issues: TLSIssue[] }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-5">
      <div className="flex items-center gap-2 mb-3">
        <Lock size={16} style={{ color: "var(--critical)" }} />
        <h2 className="text-base font-semibold" style={{ color: "var(--foreground-bright)" }}>TLS / Certificate Issues</h2>
      ...
      <div className="flex flex-col gap-1">
        {issues.map((issue, i) => (
          <div key={`tls-${i}`} className="flex items-start gap-3 rounded-lg px-3 py-2" style={{ borderBottom: "1px solid var(--border-subtle)" }}>
