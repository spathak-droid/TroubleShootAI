...
import type { SchedulingIssue } from "@/lib/types";
...
const issueTypeLabels: Record<string, string> = {
  ...
  taint_not_tolerated: "Taint Not Tolerated",
  node_affinity_mismatch: "Node Affinity Mismatch",
  pod_affinity_conflict: "Pod Affinity Conflict",
  node_selector_mismatch: "Node Selector Mismatch",
  unschedulable_node: "Unschedulable Node",
};

export function SchedulingIssuesSection({ issues }: { issues: SchedulingIssue[] }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h2 className="text-base font-semibold" style={{ color: "var(--foreground-bright)" }}>Scheduling Issues</h2>
      ...
      <div className="flex flex-col gap-1">
        {issues.map((issue, i) => (
          <div key={`sched-${i}`} className="flex items-start gap-3 rounded-lg px-3 py-2" style={{ borderBottom: "1px solid var(--border-subtle)" }}>
