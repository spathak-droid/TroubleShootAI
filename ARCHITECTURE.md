...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│  LAYER 1:       │ → Pre-ingestion scrubbing (credentials, PII, infra)
│  SCRUBBER       │   Protects storage, logs, UI, telemetry
└────────┬────────┘
         │
         ▼
┌─────────────────┐
...
└────────┬────────┘
    ...
    ▼                      ▼
┌──────────┐        ┌──────────────────┐
│ARCHAEOLOGY│        │  LAYER 2: SCRUB  │
│ENGINE     │        │  + PARALLEL AI   │
│           │        │                  │
│timeline + │        │  scrub → PodAI   │
│causal     │        │  scrub → NodeAI  │
│chains     │        │  scrub → CfgAI   │
│           │        │  + prompt guard   │
└─────┬─────┘        └──────┬───────────┘
...
│   │   │   ├── interview.py
│   │   │   └── simulation.py
│   │   └── client.py                  ← Claude API wrapper w/ retry + auto-scrub
│   │
│   ├── security/                      ← data protection layer
│   │   ├── __init__.py                ← exports BundleScrubber, SecurityPolicy
│   │   ├── models.py                  ← RedactionEntry, SanitizationReport, SecurityPolicy
│   │   ├── policy.py                  ← PolicyEngine — scrub level per source type
│   │   ├── patterns.py                ← 20+ regex detectors (credentials, PII, infra)
│   │   ├── kubernetes.py              ← K8s-aware structural scrubbers
│   │   ├── entropy.py                 ← Shannon entropy secret detector
│   │   ├── prompt_guard.py            ← Prompt injection detection + neutralization
│   │   ├── scrubber.py                ← BundleScrubber — composes all detectors
│   │   └── audit.py                   ← Audit logger for compliance
│   │
...
PHASE 6 — Security & Data Protection (~4hrs)
  □ Security models + policy engine
  □ Pattern detectors (20+ credential/PII/infra patterns)
  □ K8s structural scrubbers (pod spec, node JSON, logs)
  □ Entropy-based secret detection
  □ Prompt injection guard
  □ Scrubber core (composes all detectors)
  □ Audit logger
  □ Integration into client.py + analysts + orchestrator
  □ 15+ security tests
```
...
| Multi-layer scrubbing | Regex alone misses structural secrets and novel patterns. Hybrid approach needed. |
| Pre-ingestion + pre-LLM | Two boundaries: sanitize at storage entry AND at API exit. |
| Prompt injection defense | Logs are untrusted input inserted into LLM prompts. Adversarial by default. |
| Entropy detection | Catches secrets no known regex pattern covers — the unknown unknowns. |
| Preserve diagnostic keys | Env var names, resource names, labels, selectors are safe AND useful for analysis. |
...
Support bundles contain production cluster data. Data risks:

| Category | Examples | Risk Level |
|---|---|---|
| **Credentials** | API keys, JWT tokens, OAuth tokens, SSH keys, CI tokens, kubeconfig tokens, Docker auth | CRITICAL |
| **PII** | Email addresses, phone numbers, customer IDs, names in logs | HIGH |
| **Infrastructure** | Internal IPs, hostnames, cluster names, account IDs, S3 buckets, ARNs | HIGH |
| **Proprietary code** | Stack traces with file paths, repo URLs, code snippets in error messages | MEDIUM |
| **Prompt injection** | Malicious log entries designed to manipulate LLM behavior | HIGH |

### 10.2 Protection Layers

```
Layer 1: PRE-INGESTION SCRUBBER
├── Runs on ALL data before storage/indexing
├── Protects: storage, app logs, UI rendering, telemetry
└── Detectors: regex patterns, K8s structural, entropy

Layer 2: PRE-LLM SCRUBBER
├── Runs on every prompt before API transmission
├── Protects: LLM providers, training data concerns
├── Detectors: all Layer 1 + prompt injection guard
└── Wraps untrusted data in boundary markers

Layer 3: ACCESS CONTROL (future)
├── Who can upload bundles
├── Who can view analyses
├── Tenant isolation
└── Retention/deletion policy

Layer 4: AUDIT LOGGING
├── Records all redactions with detector + category
├── Separate from app logs (audit logs never contain raw data)
└── Compliance-ready export
```

### 10.3 Hybrid Detection Strategy

Do NOT rely on regex alone. Use all four:

1. **Regex/pattern rules** — Known secrets (JWT, AWS keys, connection strings, etc.)
2. **Structural scrubbers** — Understand K8s JSON: strip env values, preserve names
3. **Entropy detection** — Shannon entropy catches unknown high-randomness secrets
4. **Policy engine** — Different scrub levels by source type (pod spec vs log vs event)

### 10.4 What to Preserve (Diagnostic Value)

| Preserve | Redact |
|---|---|
| Env var **names** | Env var **values** |
| Resource names (pod, deploy, svc) | Connection string URLs |
| Namespace names | Internal IPs, hostnames |
| Label/selector **keys** | Label **values** with PII |
| Container image names | Internal registry URLs |
| Resource requests/limits | AWS account IDs, ARNs |
| Probe paths/ports | JWT tokens, API keys |
| Event reasons/types | Embedded credentials in messages |
| `***HIDDEN***` markers (as-is) | Everything the collector already redacted |

### 10.5 File Structure

```
bundle_analyzer/security/
├── __init__.py          # exports BundleScrubber, SecurityPolicy
├── models.py            # RedactionEntry, SanitizationReport, SecurityPolicy
├── policy.py            # PolicyEngine — scrub level per source type
├── patterns.py          # 20+ regex pattern groups (credentials, PII, infra)
├── kubernetes.py        # K8s-aware structural scrubbers
...
```python
# In BundleAnalyzerClient.complete():
sanitized_user, report = self._scrubber.scrub_for_llm(user)
# → every LLM call is automatically protected

# In PodAnalyst.analyze():
scrubbed_pod = self._scrubber.scrub_pod_json(pod_data)
# → pod specs are structurally scrubbed before prompt building

# In AnalysisResult:
sanitization_summary: str  # "Redacted 47 patterns (23 credentials, 12 PII, 12 infra)"
# → user sees what was protected
```
